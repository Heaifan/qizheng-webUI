...
// 2-pass box blur 平滑，不修改真实 heightMap
function buildCHM() {
...
// Marching Squares 绘制一条等值线
function drawLevel(ctx, level, color, lw, oX, oY, cs, CH) {
  let seg = 0, skip = 0;
  ctx.strokeStyle = color; ctx.lineWidth = lw;
  const rows = QZ.rows, cols = QZ.cols;
  for (let y = 0; y < rows - 1; y++) for (let x = 0; x < cols - 1; x++) {
    if (QZ.getWater(x,y) && QZ.getWater(x+1,y) && QZ.getWater(x,y+1) && QZ.getWater(x+1,y+1)) continue;
    if (relief(CH, x, y) < 0.035) { skip++; continue; }
...
QZ.drawContours = function(ctx) {
  ...
  for (let y = 0; y < QZ.rows; y++) for (let x = 0; x < QZ.cols; x++) {
    const h = CH[y][x]; if (h < minH) minH = h; if (h > maxH) maxH = h;
  }
  const range = Math.max(maxH - minH, 0.06), margin = range * 0.08;
  const lo = +(minH + margin).toFixed(3), hi = +(maxH - margin).toFixed(3);
  const lvls = Array.from({length:5}, (_,i) => +(lo + (hi - lo) * i / 4).toFixed(3));
  const S = [
    { lw: 0.45, a: 0.14 }, // 副线·低
    { lw: 0.9,  a: 0.30 }, // 主线·中低
    { lw: 0.45, a: 0.16 }, // 副线·中
    { lw: 0.9,  a: 0.32 }, // 主线·中高
    { lw: 1.0,  a: 0.38 }, // 高地核心线·最高
  ];
  let totalSeg = 0, totalSkip = 0;
  for (let i = 0; i < 5; i++) {
    const r = drawLevel(ctx, lvls[i], 'rgba(80,72,48,' + S[i].a + ')', S[i].lw, oX, oY, cs, CH);
    totalSeg += r.seg; totalSkip += r.skip;
  }
  ctx.restore();
  if (QZ._contourLogNeeded) {
    QZ.log('等高线: enabled=' + QZ.showContour + ' mode=smooth passes=2' +
      ' levels=[' + lvls.join(',') + ']' +
      ' major=[' + lvls.filter((_,i)=>S[i].lw>=0.9).join(',') + ']' +
      ' segments=' + totalSeg + ' flatSkip=' + totalSkip);
